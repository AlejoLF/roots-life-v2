...
export default function CarritoPage() {
  ...
  const shipping = subtotal >= SHIPPING_THRESHOLD ? 0 : 4500;
      ...
      <main className="relative z-10 pt-[58px] lg:pt-[72px] flex-1">
        ...
        {items.length === 0 ? (
          <section className="max-w-[42rem] mx-auto px-4 py-16 text-center">
            <p className="text-body text-ink-500 mb-6">Tu carrito está vacío.</p>
            <Button href="/catalogo" variant="dark">Explorar catálogo →</Button>
          </section>
        ) : (
          <section className="max-w-[75rem] mx-auto px-4 md:px-8 lg:px-12 pb-16 grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-8 lg:gap-12">
            ...
            <div className="flex flex-col gap-5">
              {items.map((item) => (
                <div key={item.id} className="grid grid-cols-[100px_1fr] md:grid-cols-[140px_1fr] gap-4 md:gap-6 pb-5 border-b border-[var(--color-border)]">
                  <Link
                    href={item.href}
                    className="bg-paper-200 bg-cover bg-center rounded-[2px] shadow-sm"
                    style={{ aspectRatio: '4/5', backgroundImage: `url("${item.image}")` }}
                    aria-label={`Ver ${item.title}`}
                  />
                  <div className="flex flex-col gap-2 min-w-0">
                    <p className="text-caption text-ink-500 m-0">{item.caption}</p>
                    <h2 className="text-lg md:text-xl font-medium leading-snug m-0">
                      <Link href={item.href} className="text-ink-900 no-underline hover:text-rust-500">{item.title}</Link>
                    </h2>
                    <p className="text-body-sm text-ink-500 m-0">{item.meta}</p>
                    <div className="mt-auto pt-3 flex items-center justify-between gap-3 flex-wrap">
                      <div className="inline-flex items-center border-[1.5px] border-[var(--color-border)] rounded-[4px] h-10">
                        <button onClick={() => updateQty(item.id, -1)} className="w-9 h-full hover:bg-ink-100" aria-label="Restar">−</button>
                        <span className="min-w-[2rem] text-center text-sm font-medium">{item.qty}</span>
                        <button onClick={() => updateQty(item.id, 1)} className="w-9 h-full hover:bg-ink-100" aria-label="Sumar">+</button>
                      </div>
                      <p className="text-body font-medium m-0">{formatARS(item.price * item.qty)}</p>
                      <button onClick={() => removeItem(item.id)} className="text-xs font-medium uppercase tracking-wide text-ink-500 underline underline-offset-[3px] hover:text-rust-700">
                        Eliminar
                      </button>
                    </div>
                  </div>
            ...
            <aside className="bg-paper-100 border border-[var(--color-border)] rounded-[4px] shadow-sm p-5 md:p-6 flex flex-col gap-4 lg:sticky lg:top-[88px] lg:self-start">
              ...
              {shipping > 0 ? (
                <div className="bg-paper-200 p-3 px-4 rounded-[2px] text-xs text-ink-700">
                  <span>Te faltan <strong>{formatARS(missingForShipping)}</strong> para envío gratis</span>
                  <div className="mt-2 h-1 bg-paper-300 rounded-full overflow-hidden">
                    <div className="h-full bg-ink-900 transition-all" style={{ width: `${progressPct}%` }} />
                  </div>
                </div>
              ) : (
                <div className="bg-moss-500 text-paper-100 p-3 px-4 rounded-[2px] text-xs font-medium uppercase tracking-wide flex items-center gap-2">
              ...
              <div className="border-t border-[var(--color-border)] pt-4 flex flex-col gap-2 text-sm">
                ...
                <div className="flex justify-between text-ink-500"><span>Envío</span><span>{shipping === 0 ? 'Gratis' : formatARS(shipping)}</span></div>
              ...
              <p className="text-stamp text-ink-500 text-center">
                Pago seguro · Mercado Pago · Transferencia
              </p>
            </aside>
          </section>
        )}
        ...
        <section className="py-16 px-4 md:px-8 lg:px-12">
          <div className="max-w-[75rem] mx-auto">
            <div className="flex justify-between items-baseline flex-wrap gap-4 mb-10">
              ...
              <Link href="/catalogo" className="text-button text-ink-900 no-underline hover:text-rust-500">Ver todo →</Link>
            </div>
